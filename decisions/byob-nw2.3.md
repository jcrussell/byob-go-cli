...
Idea: wrap two libraries behind the Progress interface (byob-nw2.1):
...
- **`charmbracelet/bubbles/spinner`** for unknown-total operations.
  ...
  lipgloss) is already paid by byob-mex.2's huh pick, so the
...
- **Bubbletea ceremony for the spinner.** bubbles/spinner is not a
  ...
  maintenance story and the alignment with byob-mex.2's rendering
  stack.
