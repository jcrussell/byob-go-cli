...
Problem: byob-dwt.3 allocated `ErrOut` to human chatter (progress,
...
Idea: the default log level is `Warn` (byob-89e.3), so in the common
...
Chatter (byob-dwt.3) is still the primary human channel. Logs are
machine-parseable opt-in.

No `--quiet`/`-q` flag. The template deliberately omits one because
the byob-dwt.3 / byob-89e.4 combination already gives users three
