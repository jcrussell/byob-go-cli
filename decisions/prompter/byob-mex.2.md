...
terminal (coordinated with byob-mex.3's `ErrNotTTY` sentinel), and
is actively maintained by a team that ships other CLI-adjacent
libraries.

Because the Prompter is a narrow interface (byob-mex.1), huh's types
never leak to callers; swapping the impl later is mechanical.

Tradeoffs:

- **Transitive dep weight.** huh pulls bubbletea + lipgloss. Against
  byob-65u.1's pure-Go minimalism the UX win is worth the weight,
...
Tests never instantiate `live` — they use `prompt.Stub` (byob-mex.4),
