...
Idea: every Prompter method checks `IO.IsStdinTTY()` first (byob-dwt.1
...
`--yes`/`-y` (see byob-mex.5) or failing with a clear message
...
wrong" to "explicitly unsupported." `--yes` (byob-mex.5) already
...
func (p *live) Confirm(ctx context.Context, msg string, def bool) (bool, error) {
    ...
    // ...prompt (see byob-mex.2 for the ctx-select pattern)
}
...
case err != nil:
    return err
case !yes:
    return ErrCancel // see byob-kny.1
}
```
